    ...
    try {
      ...
      if (!initRef.current) {
        ...
        if (fresh.length > 0 && !document.hasFocus()) {
          ...
          else showPopup(`${fresh.length} novas notificações`, 'Você tem novas notificações.', null);
        }
        lastIdRef.current = maxId;
      }
        ...
        <>
          ...
          <div className="fixed left-4 bottom-24 w-[330px] max-h-[60vh] bg-white rounded-2xl border border-gray-200 shadow-2xl z-50 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <p className="text-[13px] font-semibold text-[#0f172a]">Notificações</p>
              <div className="flex items-center gap-2">
                {unread > 0 && (
                  <button onClick={markAll} className="text-[11px] text-[#1D4ED8] hover:underline">marcar todas</button>
                )}
            ...
            {perm === 'default' && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-[#1D4ED8]/[0.06] text-[#1D4ED8] text-[12px] font-medium hover:bg-[#1D4ED8]/[0.1] transition-colors border-b border-gray-100"
              >
            ...
            {perm === 'granted' && (
              <button
                ...
                className="px-4 py-2 text-[11px] text-[#1D4ED8] hover:bg-[#1D4ED8]/[0.06] transition-colors border-b border-gray-100 text-left"
              >
              ...
              {items.length === 0 ? (
                ...
                items.map(n => (
                  <button
                    key={n.id}
                    onClick={() => openNotif(n)}
                    className={`w-full text-left px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors ${n.is_read ? '' : 'bg-[#1D4ED8]/[0.04]'}`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <p className={`text-[12.5px] ${n.is_read ? 'text-gray-600' : 'font-semibold text-[#0f172a]'}`}>{n.title}</p>
